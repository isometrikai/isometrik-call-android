        ...
        if (meetingsAlreadyNotified.contains(meetingId)) {
            ...
            try {
                ...
                IsometrikCallSdk.getInstance().getIsometrik().getExecutor().execute(() -> {
                    ...
                    IsometrikCallSdk.getInstance().getIsometrik().getRemoteUseCases().getMessageUseCases().sendMessage(new SendMessageQuery.Builder().setMessageType(1).setDeviceId(IsometrikCallSdk.getInstance().getUserSession().getDeviceId()).setMeetingId(meetingId).setBody(context.getString(R.string.ism_call_ringing_event)).setUserToken(IsometrikCallSdk.getInstance().getUserSession().getUserToken()).build(), (var1, var2) -> {
